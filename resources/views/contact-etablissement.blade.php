<x-layouts.app :title="'Contacter ' . $etablissement->titre">
    <div class="max-w-2xl mx-auto px-4 py-8">
        <h1 class="text-2xl font-bold mb-6">Contacter {{ $etablissement->titre }}</h1>

        <form method="POST" action="{{ route('contact.etablissement.send', $etablissement) }}" class="bg-white rounded-lg shadow-sm border p-6">
            @csrf
            <div class="mb-4">
                <label class="block text-sm font-medium mb-1">Votre nom</label>
                <input type="text" name="nom" value="{{ old('nom', auth()->user()?->pseudo) }}" class="w-full border rounded-lg px-3 py-2">
            </div>
            <div class="mb-4">
