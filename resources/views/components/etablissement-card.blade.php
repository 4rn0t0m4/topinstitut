...
<div class="relative bg-white rounded-lg shadow-sm border hover:shadow-md transition overflow-hidden">
    ...
    <a href="{{ $etablissement->url }}" class="relative block min-h-32 sm:min-h-40">
        {{-- Photo --}}
        <div class="absolute inset-y-0 left-0 w-32 sm:w-40 bg-gray-100 overflow-hidden">
            @if($photoUrl)
                <img src="{{ $photoUrl }}" alt="{{ $etablissement->name }}" class="w-full h-full object-cover" loading="lazy">
            @else
        ...
        {{-- Content --}}
        <div class="ml-32 sm:ml-40 min-w-0 p-4 pr-12">
