...
<div class="border rounded-lg p-3 bg-white">
    ...
    <div class="grid grid-cols-12 gap-2 items-start">
        <div class="col-span-12 sm:col-span-4">
            <label class="block text-[11px] font-medium text-gray-500 mb-1">Nom de la prestation</label>
            <input type="text" :name="`services[${item.index}][name]`" x-model="item.svc.name"
                   placeholder="Manucure, Épilation jambes..."
                   class="w-full border rounded-lg px-3 py-2 text-sm" required>
        </div>
        <div class="col-span-6 sm:col-span-3">
            <label class="block text-[11px] font-medium text-gray-500 mb-1">Catégorie</label>
        ...
        <div class="col-span-3 sm:col-span-2">
            <label class="block text-[11px] font-medium text-gray-500 mb-1">Durée (min)</label>
            <input type="number" min="5" max="600" step="5" :name="`services[${item.index}][duration_minutes]`"
                   x-model.number="item.svc.duration_minutes" placeholder="30"
                   class="w-full border rounded-lg px-3 py-2 text-sm text-center" required>
        </div>
        <div class="col-span-3 sm:col-span-2">
            <label class="block text-[11px] font-medium text-gray-500 mb-1">Prix</label>
        ...
        <div class="col-span-12 sm:col-span-1">
            <label class="block text-[11px] font-medium text-gray-500 mb-1 sm:text-center" title="Réservable en ligne">Résa</label>
            <div class="flex items-center justify-end sm:justify-center gap-3 pt-1.5">
                ...
                <input type="checkbox" :name="`services[${item.index}][is_bookable]`" value="1"
                       x-model="item.svc.is_bookable" title="Réservable en ligne" class="rounded">
    ...
    <label class="block text-[11px] font-medium text-gray-500 mt-2 mb-1">Description (optionnel)</label>
    <textarea :name="`services[${item.index}][description]`" x-model="item.svc.description"
              placeholder="Détails affichés sous le nom de la prestation"
              rows="2" class="w-full border rounded-lg px-3 py-2 text-sm"></textarea>
</div>
